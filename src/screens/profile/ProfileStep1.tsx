    ...
    <View style={[{ flex: 1, backgroundColor: colors.greyLight },]}>
      <ScrollView>
        ...
        <View style={{ marginHorizontal: 20 }}>
          ...
          <TextWrapper children='What services do you provide?' fontType={'semiBold'} style={{ fontSize: 16, marginTop: 13, marginBottom: 45, color: colors.black }} />
            ...
            ListFooterComponent={() => {
              return (
                <View style={{ flex: 1, marginTop: 40, alignItems: 'center', justifyContent: 'center', }}>
                  {isLoading && <ActivityIndicator size={'large'} color={colors.parpal} />}
                </View>
              )
            }}
            showsVerticalScrollIndicator={false}
            renderItem={({ item, index }) => <CategoryList categoryName={item.name} catId={item?.id} />}
            ListEmptyComponent={() => <Text style={[{ color: '#000', alignSelf: 'center', marginTop: 100, fontFamily: commonStyle.fontFamily.regular },]}>{!isLoading ? 'No service found' : ''}</Text>} />
          {isAddService ? <>
        ...
        <View style={[generalStyles.rowBetween, { marginHorizontal: 20, marginBottom: 35 }]}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
          {category?.length ? category?.map((item: any, index: any) => {
            return (
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 13, marginHorizontal: 20 }}>
                <View key={index} style={{ paddingHorizontal: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: colors.lightBlack, height: 30, width: 'auto', borderRadius: 5, }}>
                  <TextWrapper
                    fontType={'semiBold'}
                    style={{
                      fontSize: 12,
                ...
                <TouchableOpacity onPress={() => { dispatch(removeCategory(item)) }}>
                  <Image source={images.cross} style={{ width: 15, height: 15, marginLeft: 20, tintColor: '#000' }} />
                </TouchableOpacity>
              </View>
            )
          }): null}
